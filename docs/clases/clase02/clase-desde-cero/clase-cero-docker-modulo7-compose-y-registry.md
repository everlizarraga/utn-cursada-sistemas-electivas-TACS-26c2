...
- 🟡 Sobre `depends_on`, una honestidad: garantiza el **orden de arranque** (primero db, después app), pero "arrancó" no es "está lista" — un motor de base tarda unos segundos en aceptar conexiones después de arrancar. Para el TP alcanza con saberlo (y que la app reintente la conexión); las soluciones finas existen y no son de esta serie.
